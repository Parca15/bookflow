...
-- Migrar datos existentes: si tiene NIT (10 dígnitos) usar NIT, si no usar CC
UPDATE companies SET document_type = 'NIT' WHERE LENGTH(REPLACE(document_number, '-', '')) = 10 AND document_number ~ '^[0-9]+$';
UPDATE companies SET document_type = 'CC' WHERE document_type = 'NIT' IS NULL OR document_type = 'NIT';
